...
/**
 * Same as above, but caches the result in the session so folder discovery
 * (a slow imap_list scan of your whole mailbox) only ever runs once per
 * login instead of on every single page load. This is what was making the
 * Sent tab open two IMAP connections every time.
 */
function get_sent_folder_name(array $acc): ?string {
    $cacheKey = 'sent_folder_' . $acc['id'];
    if (array_key_exists($cacheKey, $_SESSION)) {
        return $_SESSION[$cacheKey];
    }
    $imap = imap_connect_account($acc, 'INBOX');
    if (!$imap) return null;
    $folder = imap_guess_sent_folder($imap, $acc);
    imap_close($imap);
    $_SESSION[$cacheKey] = $folder; // cache even a null result, so a missing folder isn't re-scanned every time
    return $folder;
}
 ...
 * Fetches all overviews in a SINGLE round-trip (instead of one call per message) —
 * this is the main thing that was making Inbox/All Mail/Sent slow to load.
 * Attachment presence is intentionally NOT checked here (that requires a separate
 * imap_fetchstructure() call per message); it's only checked when you open a message.
 */
function imap_message_list($imapStream, string $criteria = 'ALL', int $limit = 50): array {
    ...
    // One batched fetch for every UID at once.
    $overviews = @imap_fetch_overview($imapStream, implode(',', $uids), FT_UID);
    if (!$overviews) return [];
    ...
    $messages = [];
    foreach ($uids as $uid) {          // preserves the newest-first order
