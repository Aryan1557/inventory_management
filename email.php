...
$create_tables = [
    "CREATE TABLE IF NOT EXISTS email_folders (
        ...
        subject VARCHAR(500),
        body LONGTEXT,
        folder ENUM('inbox', 'starred', 'snoozed', 'important', 'spam', 'sent'),
        is_read TINYINT(1) DEFAULT 0,
        is_starred TINYINT(1) DEFAULT 0,
        has_attachment TINYINT(1) DEFAULT 0,
        message_date DATETIME,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY unique_message (email_address, message_uid)
    )",
...
if (empty($_SESSION['email_address'])) {
    ...
    if ($user_query && mysqli_num_rows($user_query) > 0) {
        ...
        $email = trim($user['email_id']);
        $_SESSION['email_address'] = $email;
        $_SESSION['current_folder'] = 'inbox';

        if (strpos($email, '@gmail.com') !== false) {
            $_SESSION['email_provider'] = 'gmail';
        } elseif (strpos($email, '@rediffmail.com') !== false) {
            $_SESSION['email_provider'] = 'rediff';
        } else {
            $_SESSION['email_provider'] = 'gmail';
        }
...
// ==================== FOLDER SWITCH ====================
if (isset($_GET['folder']) && !isset($_POST['send_email']) && !isset($_POST['select_provider']) && !isset($_GET['logout_email']) && !isset($_GET['fetch'])) {
    $allowed = ['inbox', 'sent', 'important', 'spam'];
...
if (isset($_GET['fetch']) && $_GET['fetch'] == 'inbox' && !empty($_SESSION['email_provider'])) {
    ...
    if (!function_exists('imap_open')) {
        $notification = 'IMAP extension not enabled! Enable php_imap in php.ini';
        ...
        if (!$inbox) {
            ...
            $notification = 'IMAP connection failed: ' . $error . '. Check your email/password and ensure IMAP is enabled in your email settings.';
            $notification_type = 'error';
            $redirect = true;
        } else {
            ...
            for ($i = $email_count; $i >= $start; $i--) {
                ...
                if ($header && isset($header->from[0])) {
                    ...
                    $check_query = mysqli_query($conn, "SELECT id FROM email_folders WHERE email_address='$esc_email' AND message_uid='$esc_uid' LIMIT 1");
                    ...
                    if (!$already_exists) {
                        ...
                        $insert_sql = "INSERT INTO email_folders (email_address, message_uid, from_email, from_name, subject, body, folder, is_read, message_date, created_at) 
            ...
            if ($fetch_count > 0) {
                $notification = "Successfully fetched $fetch_count new emails!";
                $notification_type = 'success';
            } else {
                $notification = 'No new emails found. Inbox is up to date.';
                $notification_type = 'success';
            }
            $redirect = true;
        }
...
if (isset($_POST['send_email']) && !empty($_SESSION['email_provider'])) {
    ...
    try {
        ...
        $notification = 'Send failed: ' . $mail->ErrorInfo;
        $notification_type = 'error';
    }
    ...
    VALUES (
        ...
        if (!$result) {
            error_log("Sent Save Error: " . mysqli_error($conn));
        }
...
if (!empty($_SESSION['email_provider'])) {
    ...
    $email_addr = mysqli_real_escape_string(
        $conn,
...
<head>
    ...
    <style>
        ...
        * {
        ...
        #emailViewer .close-viewer:hover {
            transform: translateY(-2px);
            box-shadow: 0 8px 20px var(--orange-shadow);
        }
        ...
        <?php if (!$is_logged_in): ?>
            <div class="provider-selection">
                <div class="provider-card">
                    <h1 class="provider-title">📧 Email Management</h1>

                    <?php if (!$imap_available): ?>
                        <div class="imap-warning">
                            ⚠️ <strong>IMAP not enabled!</strong> Enable <code>extension=imap</code> in php.ini to fetch real emails.
                        </div>
                    <?php endif; ?>
                    ...
                    <form method="POST" action="email.php">
                        ...
                        <input type="password" name="password" class="form-input" placeholder="Password / App Password" required>
                        <button type="submit" name="select_provider" class="btn btn-primary">🔗 Connect</button>
                    </form>
                </div>
            </div>
        <?php else: ?>
            <div class="email-client">
                <div class="email-sidebar">
                    <div class="provider-info">
                        <span style="font-size:20px;"><?= $email_config[$provider]['icon'] === 'G' ? '🔴' : '🔵' ?></span>
                        <div>
                            <strong style="color:var(--orange-primary);"><?= $email_config[$provider]['name'] ?></strong>
                            <small style="display:block;color:var(--secondary);"><?= htmlspecialchars($_SESSION['email_address']) ?></small>
                        </div>
                    ...
                    <ul class="folder-list">
                        <a href="email.php?folder=inbox" class="folder-item <?= $current_folder == 'inbox' ? 'active' : '' ?>">
                            <span class="folder-icon">📥</span> Inbox <span class="folder-badge"><?= $inbox_count ?></span>
                        </a>
                        <a href="email.php?folder=sent" class="folder-item <?= $current_folder == 'sent' ? 'active' : '' ?>">
                            <span class="folder-icon">📤</span> Sent
                        </a>
                        <a href="email.php?folder=important" class="folder-item <?= $current_folder == 'important' ? 'active' : '' ?>">
                            <span class="folder-icon">⭐</span> Important
                        </a>
                ...
                <div class="email-main">
                    <div class="email-header">
                        ...
                        <span style="margin-left:auto;font-size:13px;color:var(--secondary);"><?= count($emails) ?> emails</span>
                    </div>
                    <div class="email-list">
                        <?php if ($current_folder == 'drafts'): ?>
                            ...
                            <?php else: ?>
                                ...
                                <?php endforeach; ?>
                            <?php endif; ?>
                        <?php elseif ($current_folder == 'inbox' && empty($emails)): ?>
                            <div class="no-emails">
                                <div class="icon">📥</div>
                                <h3>Inbox Empty</h3>
                                <p>Click <strong style="color:var(--orange-primary);">"Fetch Inbox Emails"</strong> to download your real emails</p>
                            </div>
                        <?php elseif (empty($emails)): ?>
                            <div class="no-emails">
                                <div class="icon">📧</div>
                                <h3>No emails</h3>
                                <p>Your <?= $current_folder ?> folder is empty</p>
                            </div>
                        <?php else: ?>
                            <?php foreach ($emails as $email): ?>
                                <div class="email-list-item <?= empty($email['is_read']) ? 'unread' : '' ?>"
                                    onclick='viewEmail(<?= json_encode($email, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>
            ...
            <div class="compose-modal" id="composeModal">
                ...
                <form method="POST" action="email.php">
                    ...
                    <div class="compose-actions">
                        <button type="submit" name="send_email" class="btn btn-send">📤 Send</button>
            ...
            <div id="emailViewer">
                ...
                <button class="close-viewer" onclick="document.getElementById('emailViewer').style.display='none'">✕ Close</button>
            </div>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        function viewEmail(email) {
            ...
            content.innerHTML = `
                ...
                <p><strong>Date:</strong> ${email.message_date || ''}</p>
                <hr>
